      ...
      <?php if($add_button || $del_button || $search_button): ?><div class="table-bar">
        <div class="fl">
          <?php if(empty($model["extend"])): ?><div class="tools">
				...
				<?php if($nodel == 0): ?><if condition="$del_button">
				<?php $del_url || $del_url = U('del?model='.$model['id']); ?>
				<button class="btn ajax-post confirm" target-form="ids" url="<?php echo ($del_url); ?>">删 除</button><?php endif; endif; ?>               
			</div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
